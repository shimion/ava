...
( function($) {
	...
	**/
	fusionPreview.updateContentBoxesPreview = function( thisRef, model, subElements ) {
		...
		innerHtml			= innerHtml.replace( $(innerHtml).find('font.content_boxes_columns').html() , subElements[1].value );
	...
	**/
	fusionPreview.updateImageFramePreview = function( thisRef, model, subElements ) {
		...
		innerHtml 			= innerHtml.replace( $(innerHtml).find('div.img_frame_section').html() , '<img src="'+subElements[6].value+'">' );
	...
	**/
	fusionPreview.updateTestimonialPreview = function( thisRef, model, subElements ) {
		...
		for ( i = 0; i < totalElements; i ++) {
			...
			//if company exists
			if( element[3].value != '' ) {
				previewData+= ', '+element[3].value+'<br>';
			}
		}
